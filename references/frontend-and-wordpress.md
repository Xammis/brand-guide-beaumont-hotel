...
**Recommendation: do not authenticate for v1.0.** An application password or authenticated read-only REST session would expose editor registries, customized template records, and raw global-style records, but those would add implementation metadata rather than materially change the documented brand standards. Revisit authenticated reads only if a later task requires exact editor synchronization, template migration, or block-pattern portability.
